...
@interface GYAddNewCaseSixthVC ()<LDActionSheetDelegate,LDImagePickerDelegate,UICollectionViewDelegate,UICollectionViewDataSource>
...
- (IBAction)nexBtnClick:(id)sender {
    if (self.imageDataArr.count == 0) {
        [MBProgressHUD showError:@"请先上传证件材料"];
    } else {
        GYAddNewCaseSeventhVC *seventhVC = [[GYAddNewCaseSeventhVC alloc]init];
        seventhVC.ajbsStr = self.ajbsStr;
