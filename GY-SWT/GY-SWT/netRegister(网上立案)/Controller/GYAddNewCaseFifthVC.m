...
@interface GYAddNewCaseFifthVC ()<LDActionSheetDelegate,LDImagePickerDelegate,UICollectionViewDelegate,UICollectionViewDataSource>
...
- (void)setUI {
    ...
    self.myCollectionView.layer.masksToBounds = YES;
...
- (IBAction)nexBtnClick:(id)sender {
        if (self.imageDataArr.count == 0) {
            [MBProgressHUD showError:@"请先上传诉讼书"];
        } else {
            GYAddNewCaseSixthVC *sixthVC = [[GYAddNewCaseSixthVC alloc]init];
            sixthVC.ajbsStr = self.ajbsStr;
